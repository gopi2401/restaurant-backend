...
const orderCreate = async (req, res) => {
    try {
        ...
        var total_price = menus.reduce((acc, crr) => (acc + crr.menu_price * crr.menu_quantity), 0);
        if (price != total_price) {
            res.status(500).json(
                { message: "Price does not match!" }
            )
        }
        const data = await Order.createOrder({ customer_name, quantity: total_quantity, price: total_price, });
        menus.map(async (item) => { await Order_items.createOrderItem({ order_id: data, ...item }); })
...
module.exports = { orderCreate, orderGetAll, orderAndItemsGetAll, orderGetById, orderUpdateById, orderDeleteById }