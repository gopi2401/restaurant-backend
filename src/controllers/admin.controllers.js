...
const userUpdateById = async (req, res) => {
    try {
        const { phone, password } = req.body;
        const hashedPassword = await bcrypt.hash(password, 10);
        const data = await User.updateUser(req.params.id, { phone, password: hashedPassword });
        if (data) {
...
module.exports = { userCreate, userGetById, userGetAll, userUpdateById, userDeleteById }